...
$stmt=$user->runQuery("select * from matching_jobs where userid='$uid'");
$stmt->execute();
$data=$stmt->fetch(PDO::FETCH_ASSOC);
$match_ids=$data['match_ids'];

$query="select id,job_title,prefer_skill,job_desc,city,job_type,company_name,post_date from meem_jobs where id in ($match_ids) and status='1'";
...
// for location
   ...
   // for keyword
      if($_POST['keyword']){  $_SESSION['keyword']=$_POST['keyword']; } 
...
// pagination
    ...
    $limit=10;
    $start = ($page-1)*$limit;
    $paginationlink = "match_result.php?page=";
...
?>
        ...
        <?php
            ...
            ?>
            <a href="matched-job.php?token=<?php echo base64_encode($row['id']);?>" target="_blank">      
       <li>       
 ...
 <?php }}
     ...
     echo '<div class="alert alert-info text-center" style="padding:100px 0px">No Job Found for given Criteria.</div>';   
    }
  } ?>
