<?php
 ...
 $user = new USER();
  ...
  if (empty($_SESSION['success'])) {
	 ...
	 $stmt=$user->runQuery("select duration from renewal_tble where price_inr='$amount'");
	 $stmt->execute();
	 ...
	 $amount='INR'.$amount;
     $stmt=$user->runQuery("update job_users set paid='Y', expire='', paid_date='$pdate', expiry_date='$expirydate', amount='$amount', plan='$plan' where id='$userid'");
     $stmt->execute();
 ...
 // insert payment details in payment table
  ...
  $date=date('Y-m-d');
   $stmt=$user->runQuery("insert into payment_tble(userid,profileId,name,email,mobile,payfor,payby,currency,amount,pay_date) values('$userid', '".$data['profileId']."', '".$data['name']."', '".$data['email']."', '$mobile','renewal','Job seeker',inr','$amount','$date')");
   $stmt->execute();
