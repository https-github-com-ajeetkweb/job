<?php
   ...
   $user->redirect("index.php?msg=Please Login First");
    ...
    $admin=$_SESSION['name'];
     ...
     {
     $job_id=$_GET['job_id'];
     $sql=$user->runQuery("update meem_jobs set status='1' where id='$job_id'");
     $sql->execute(); 
     
     ?>
<script>
   alert('Job approved successfully.');
   window.location.href='newjob-approval.php';
...
<?php
   ...
   {
   $job_id=$_GET['job_id'];
   $sql=$user->runQuery("delete from meem_jobs  where id='$job_id'");
   $sql->execute(); 
                        ...
                        <div class="panel-body table-responsive">
                           <table class="table convert-data-table table-striped">
                              ...
                              <tbody class="text-center">
                                 <?php
                                    $i=1;
                                    $stmt=$user->runQuery("select * from meem_jobs where status=0");                                         
                                    $stmt->execute();
                                    $data=$stmt->fetchAll();
                                    foreach ($data as $row)
                                    {
                                 ...
                                 <tr>
                                    ...
                                    <td>
                                       <a href="newjob-approval.php?view=job&job_id=<?php echo $row['id'];?>" class="btn btn-sm btn-success" target="_blank">View</a>
                                    </td>
                                 </tr>
                                 <?php $i++; } ?>
                              </tbody>
